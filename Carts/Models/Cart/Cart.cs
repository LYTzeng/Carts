    ...
    public class Cart : IEnumerable<CartItem> //購物車類別
    {
        ...
        public bool AddProduct(int ProductId)
        {
            ...
            {   //不存在購物車內，則新增一筆
                ...
                {
                    var product = (from s in db.CLOTHES 
                                  where s.Id == ProductId 
                                  select s).FirstOrDefault();
                    if( product != default( Models.CLOTHES ) )
                    {
        ...
        private bool AddProduct(CLOTHES product)
        {
            //將Product轉為CartItem
        ...
        public List<Models.ORDER_ITEM> ToOrderDetailList(int orderId)
        {
            var result = new List<Models.ORDER_ITEM>();
            foreach(var cartItem in this.cartItems)
            {
                result.Add(new Models.ORDER_ITEM() { 
                    ClothesId = cartItem.Id,
                    Cost = (int)cartItem.Price,
